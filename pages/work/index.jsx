  ...
  return (
    <section className="padding-container max-container py-12 flex flex-col flexCenter md:flex-row md:gap-8 lg:gap-20 xl:gap-28">
      ...
      >
        ...
        <p className="text-center max-w-md mx-auto text-gray-20 text-sm sm:text-base md:text-lg">Desde aplicaciones web y móviles hasta soluciones de backend complejas, cada trabajo demuestra mi habilidad para enfrentar desafíos técnicos.</p>
      </motion.div>
      {/* Swiper */}
      <motion.div
                ...
                {slide.images.map((image, i) => (
                  <div key={i} className='flexCenter'>
                    <div className='relative overflow-hidden group rounded-lg cursor-pointer'>
                      ...
                      {/* title */}
                      <Link target="_blank" href={image.link} className='absolute top-[100%] left-1/2 text-2xl -translate-x-1/2 -translate-y-1/2 group-hover:top-1/2 transition-all duration-500 opacity-0 group-hover:opacity-100'><FaExternalLinkAlt /></Link>
                    </div>
                  </div>
                ))}
              </div>
